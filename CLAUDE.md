...
- **Twitch 1440p is HEVC, and Chromium has no software HEVC decoder.** On NVIDIA, VA-API is blocked by default
  ...
  "Device or settings issue". Measured 2026-09-19 one flag at a time in fresh profiles with the same policies:
  `--enable-features=VaapiOnNvidiaGPUs` alone makes an HEVC file play in a real window (182 frames/6 s, 0 dropped,
  `NotSupportedError` without it); `--use-gl=angle --use-angle=gl` is only needed *headless* and proves nothing
  about the session, `LIBVA_DRIVER_NAME=nvidia`, `--ignore-gpu-blocklist` and `AcceleratedVideoDecodeLinuxGL` change
  nothing. Media in a background tab stalls at `readyState 0` — activate the tab before measuring playback. The flag
  rides on the same overriding desktop file as `--force-dark-mode`, so an existing install needs its
  `~/.local/share/applications/brave-browser.desktop` patched by hand — `firstlogin-setup.sh` does not run again.
...
- **Brave reads only fontconfig cache format 11** (its own bundled fontconfig; Fedora's 2.17 writes cache-9), so it
  ...
  *any* cache once the font directory's mtime is 0, and on ostree everything under `/usr` is mtime 0. Seen 2026-09-06:
  ...
  Brave run supplies cache-11); with mtime 0 the image's cache beats the user's. Apps carrying yet another
